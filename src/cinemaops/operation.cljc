(ns cinemaops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (CinemaOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the CinemaOpsGovernor (:govern)
  ...
  human theater operations coordinator. The approver resumes with
...
(defn build
  ...
  `cinemaops.store/Store`).
  opts:
    :advisor      -- a `cinemaops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CinemaOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; CinemaOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :screening-id (:screening-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :screening-id (:screening-id request) :by (:by approval)}]}
            {:disposition :hold
